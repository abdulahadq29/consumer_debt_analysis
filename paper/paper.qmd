---
title: "Impact of Bank of Canada's Monetary Policy on Consumer Debt Levels"
author: 
  - Abdul Ahad Qureshi
...
Data analysis for this paper is performed in R [@citeR], and additional help is provided by libraries such as `dplyr` [@dplyr], `ggplot2` [@ggplot], `ggrepel` [@ggrepel], `tidyverse` [@thereferencecanbewhatever], `kableExtra` [@kableextra], `knitr` [@knitr], `haven` [@haven], 'readr' [@readr], `stargazer`[@stargazer], `psych` [@psych], `scales` [@scales], `RColorBrewer` [@rcolorbrewer], `gridExtra` [@gridextra], `here` [@here].
...
The graph @fig-regression visualizes the correlation between consumer debt and short-term interest rates.
